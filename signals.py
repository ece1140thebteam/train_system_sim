...
class signals(QObject):
  ...
  send_CTC_switch_position_signal = pyqtSignal(str, list) # Line, block, position (0=normal, 1=reverse)

  # Send maintenance mode from CTC to Track Controller
  send_CTC_maintenance_mode_signal = pyqtSignal(str, list) # Line, block, mode (0=disabled, 1=enables)

  # Send suggested speed from CTC to Track Controller
  send_CTC_suggested_speed = pyqtSignal(str, list) # Line, block, speed

  # Send track occupancy from Track Controller to CTC
  send_TrackController_track_occupancy = pyqtSignal(str, list) # Line, block, occupancy (0=open, 1=occupied)

  # Send block authority from CTC to Track Controller
  send_CTC_authority = pyqtSignal(str, list) # Line, block, authority (0=no, 1=yes)

  # Send crossing status from Track Controller to CTC
  send_TrackController_crossing = pyqtSignal(str, list) # Line, block, status (0=deactivated, 1=activated)

  # Send track failure from Track Controller to CTC
  send_TrackController_failure = pyqtSignal(str, list) # Line, block, failure type
